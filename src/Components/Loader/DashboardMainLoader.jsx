const DashboardMainLoader = () => {
  return (
    <div className="flex flex-col items-center justify-center h-full text-center px-6 space-y-6">
      {/* Spinner Circle + Logo */}
      <div className="relative w-15 h-15">
        <div className="absolute inset-0 rounded-full border-4 border-gray-200 border-t-[#009EE4] animate-spin" />
        <img
          src="/postpad-icon.png"
          alt="postpad-icon"
          className="absolute inset-0 ml-[8.3px] w-10 h-10 m-auto rounded-full"
        />
      </div>

      {/* Heading */}
      <h1 className="text-xl font-bold text-gray-700 dark:text-white max-w-xl">
        Welcome To Dashboard
      </h1>

      {/* Subheading */}
      <p className="text-[16px] text-gray-500 max-w-xl -mt-3.5 dark:text-white">
        Monitor, manage, and update all of your  <br /> operations in one
        place
      </p>

      {/* Optional Loading Animation */}
      <p className="text-xs text-gray-400 flex items-center gap-1 -mt-4.5 dark:text-white">
        Loading Dashboard
        <span className="animate-bounce text-[#009EE4] text-3xl -mt-1.5">
          .
        </span>
        <span className="animate-bounce [animation-delay:200ms] text-[#009EE4] text-3xl -mt-1.5">
          .
        </span>
        <span className="animate-bounce [animation-delay:400ms] text-[#009EE4] text-3xl -mt-1.5">
          .
        </span>
      </p>
    </div>
  );
};

export default DashboardMainLoader;